    ...
    try {
      ...
      for (int i = 0; i < images.length; i++) {
        ...
        CloudinaryResponse res = await cloudinary.uploadFile(
          CloudinaryFile.fromFile(images[i].path,folder: name),//creating folder for each product and stroing product images in that folder.
        );
        imageUrls.add(res.secureUrl);//it's the same as download url that we get in firebase.
      }
